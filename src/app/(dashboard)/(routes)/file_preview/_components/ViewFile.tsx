...
const ViewFile = ({ Data, SavePassword }) => {
  ...
  const CopyUrl = async () => {
    ...
    console.log("Copied");
    return setCopied(<Alert msg={"URL Copied!"} color={"green"} />);
  };
      ...
      <div className=" flex justify-center lg:flex-row flex-col sm:p-10 p-4  gap-4 items-center h-[calc(100vh-64px)]">
        ...
        <div className=" xl:w-[26rem] w-full lg:w-5/6  h-80 shadow-[0px_0px_10px_-2px]  rounded-md shadow-slate-400  p-5">
          ...
          <div className="flex gap-[6px] text-gray-700 items-center   ">
            <h1 className=" w-5/6 overflow-hidden border-gray-400 border rounded p-2">
              {Data?.shortUrl}
            </h1>
            <button
              onClick={CopyUrl}
              className=" rounded-md cursor-pointer ml-1 bg-primary text-white py-2 w-16 flex justify-center"
            >
              <Copy />
            </button>
          </div>
